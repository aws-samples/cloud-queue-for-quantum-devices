...
function processAuthRequest(event, tokenIssuer, awsAccountId, apiOptions, callback) {
  ...
  jwt.verify(token, pem, {issuer: tokenIssuer}, function (err, payload) {
    if (err) {
      ...
      const scopes = decodedJwt.payload['scope'] ? decodedJwt.payload['scope'].split(" ") : [];
      
      // Researcher
      if (decodedJwt.payload['cognito:groups'] &&
          decodedJwt.payload['cognito:groups'].includes("AllDevices")) {
        policy.allowMethod(AuthPolicy.HttpVerb.GET, '/devices');
        policy.allowMethod(AuthPolicy.HttpVerb.GET, '/devices/*');
        policy.allowMethod(AuthPolicy.HttpVerb.PUT, '/devices/*');
        policy.allowMethod(AuthPolicy.HttpVerb.GET, '/workloads');
        policy.allowMethod(AuthPolicy.HttpVerb.POST, '/workloads');
        policy.allowMethod(AuthPolicy.HttpVerb.GET, '/workloads/*');
      }
      
      // Admin
      if (decodedJwt.payload['cognito:groups'] &&
          decodedJwt.payload['cognito:groups'].includes("Admin")) {
        policy.allowMethod(AuthPolicy.HttpVerb.ALL, '*');
      }
      
      // Device
      if (scopes.includes("workloadapi/getdevice")) {
        policy.allowMethod(AuthPolicy.HttpVerb.GET, '/devices/*');
      }
      
      if (scopes.includes("workloadapi/updatedevice")) {
        policy.allowMethod(AuthPolicy.HttpVerb.PUT, '/devices/*');
      }
      
      if (scopes.includes("workloadapi/putdevicework")) {
        policy.allowMethod(AuthPolicy.HttpVerb.PUT, '/devices/*');
      }
      
      if (scopes.includes("workloadapi/getdevicework")) {
        policy.allowMethod(AuthPolicy.HttpVerb.GET, '/devices');
      }
...
exports.handler = (event, context, callback) => {
  ...
  let userPoolURI = 'https://cognito-idp.' + AWS_REGION
